...
@bot.command(aliases=["winls","lswin"], help="Lists Alt+Tab windows grouped by process name.", usage="$winlist")
...
if __name__ == "__main__":
    try:
        if "--version" in sys.argv:
            print(version, flush=False)
            sys.exit(0)
        else:
            bot.run(TOKEN, reconnect=True)
    
    except discord.errors.LoginFailure:
        print("\n\033[91mError: Invalid bot token.\033[0m", flush=False)
        sys.exit(1)
    
    except KeyboardInterrupt:
        print("\n\033[91mExiting...\033[0m", flush=False)
